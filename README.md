...
The active app flow is `start.py -> key4ce.ui.app.App` with rich terminal screens for menu, typing, results, and analytics.
...
## Analytics Recommendation (Groq)

The in-app analytics screen includes a concise 1-2 line recommendation.

- If `GROQ_API_KEY` is set, Key4ce requests a short recommendation from Groq.
- If unavailable (no key/network), Key4ce uses a deterministic local recommendation fallback.
