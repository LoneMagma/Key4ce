...
class TypingEngine:
    ...
    Cursor advances on each printable key to preserve typing flow.
    Supports callbacks used by screens and compatibility methods expected
    by tests.
    """
    ...
    def handle_backspace(self) -> None:
        ...
        if self.position > 0:
            self.position -= 1
            self._state.position = self.position
            self._state.error_positions.discard(self.position)
        self.has_error = False
        self.last_error_char = ""
    ...
    def process_key(self, key: str) -> KeystrokeResult:
        ...
        last_correct = bool(self.timeline.keystrokes[-1].is_correct) if self.timeline.keystrokes else False

        if self.is_complete:
            return KeystrokeResult.TEXT_COMPLETE
        if self.position > before_pos and last_correct and key == " ":
            return KeystrokeResult.WORD_COMPLETE
        if self.position > before_pos and last_correct:
    ...
    def char_state(self, index: int) -> str:
        if index < self.position:
            return "typed_error" if index in self._state.error_positions else "typed"
        if index == self.position:
